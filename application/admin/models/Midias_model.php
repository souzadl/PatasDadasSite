...
class Midias_model extends CI_Model {	
	...
	function numMidias ()
	{
		$this->db->select('*')->from('midias');
		
		return $this->db->count_all_results();
	}
	
	function getMidias ($offset = 0)
	{		
		$this->db->flush_cache();
		$this->db->select('*')->from('midias')->order_by('titulo', 'asc');
	...
	{		
		$where = array ('id_midia' => $id_midia);		
		$this->db->select('*')->from('midias')->where($where);

		$query = $this->db->get();			
		return $query->row();
	}
	
	function setMidia ($data, $id_midia = "")
	{
		if ($id_midia)
		{
			$where = array ('id_midia' => $id_midia);
			...
			else
			{
				...
				//INICIO UPLOAD CAPA
				//=====================================================================
				$title 	= $this->utilidades->sanitize_title_with_dashes($data['titulo']);
				$dt_hj 		= date("dmyHis");
				
				$config['upload_path']		= './assets/uploads/midias';
		        ...
		        {
		        	$imagem = $this->getMidia($id_midia);
		        	if($imagem->imagem)
					{
						unlink('./assets/uploads/midias/' . $imagem->imagem);
					}
				...
				//INICIO UPLOAD CAPA
				//=====================================================================
				$titleF 	= $this->utilidades->sanitize_title_with_dashes($data['titulo']);
				$dt_hj 		= date("dmyHis");
				
				$configF['upload_path']		= './assets/uploads/midias';
		        ...
		        if ($this->upload->do_upload('arquivo'))
		        {
		        	...
		        	if($imagem->arquivo)
					{
						unlink('./assets/uploads/midias/' . $imagem->arquivo);
					}
		        	$file_data = $this->upload->data();
		...
		else
		{
			...
			$this->session->set_flashdata('resposta', 'Mídia adicionada com sucesso (:');
	...
	{		
		...
		else
		{
	        ...
	        $this->session->set_flashdata('resposta', 'Midia excluído com sucesso (:');            
		}
	}
